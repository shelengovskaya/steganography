...
def get_M(image, message, step):
	dc_coef = get_dc_coef(image)
	# print(dc_coef[:10])
	Z = get_Z(dc_coef, step)
	...
	for i in range(len(dc_coef)):
		if index >= len(message):
			...
			continue
		QA, QB = get_QA_QB(dc_coef, Z, step)
		if message[index] == '0':
			min_val = 100000000
			min_k = 0
			for k in range(Z):
...
def extract(dc_coef, length, step):
	result = ''

	for i in range(min(length, len(dc_coef))):
		W = ceil(dc_coef[i]/step) % 2
		result += str(W)
...
def main():
	...
	for step in range(5, 100, 15):
		...
		result_message = extract(new_dc_coef, len(message), step)
